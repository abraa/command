...
if (!defined('IN_INIT_7NJ3319RLgeI10'))
{
    die('Hacking attempt');
}
...
/*    	//暂时屏蔽1-23号前的订单列表显示
		$sql = strtolower($sql);
		if (strstr($sql,'order_info') && strstr($sql,'select'))
		{
			if (!admin_priv_result('order_explort',false))
			{
				$sql = str_replace('where','where add_time > 1295769599 and ',$sql);
			}
		}//end*/
...
/* 仿真 Adodb 函数 */
    ...
    /**
     * 批量插入数据, $batch_field_values必须是二维数组
     * @param $table
     * @param $batch_field_values
     * @param int $count                一次插入数量
     * @return bool
     */
    function batchInsert($table,$batch_field_values,$count = 100){
        if(!is_array($batch_field_values[0])||$count <=0){
            return false;
        }
        $field_names = $this->getCol('DESC ' . $table);
        $sql = '';
        $fields = array_intersect($field_names,array_keys($batch_field_values[0]));  //取存在的字段
        if(empty($fields)){
            return false;
        }
        $arr = array();
        $i = 0;
        foreach($batch_field_values as $field_values){
            $i++;
            $values = array();
     ...
     * @return array|bool
     */
    function get_addslashes_all($sql){
        $res = $this->query($sql);
        if ($res !== false)
        {
            $arr = array();
