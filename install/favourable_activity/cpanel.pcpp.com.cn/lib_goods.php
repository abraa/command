...
function get_intro_list()
{
    return array(
        ...
        'is_promotion'     => $GLOBALS['_LANG']['is_promotion'],
 ...
 * @return  void
 */
function handle_gallery_image($goods_id, $image_files, $image_descs, $target_sub_dir='')
{
    /* 是否处理缩略图 */
    ...
    foreach ($image_descs AS $key => $img_desc)
    {
        ...
        if ($flag)
        {
            ...
            /* 重新格式化图片名称 */
            $img_original = reformat_image_name('gallery', $goods_id, $img_original, 'source', $target_sub_dir);
            $img_url = reformat_image_name('gallery', $goods_id, $img_url, 'goods', $target_sub_dir);
            $thumb_url = reformat_image_name('gallery_thumb', $goods_id, $thumb_url, 'thumb', $target_sub_dir);
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        $filter['is_promotion']   = empty($_REQUEST['is_promotion']) ? '' : trim($_REQUEST['is_promotion']);
        $filter['is_delete']        = $is_delete;
        $filter['real_goods']       = $real_goods;

            /* 商品属性查询商品 */
        if (! empty ( $filter ['cat_id'] )) {
            $where .= ' AND goods_id in (' . get_cat_goods ( $filter ['cat_id'] ) . ') OR cat_id  = ' . $filter ['cat_id'];
        }
		...
		$check_skip_link = check_skip_link();  //检查是否可以操作"跳转链接"
		
        $sql = "SELECT is_promotion,goods_id, goods_name".($check_skip_link ? ',skip_link' : '').", goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot,is_package, sort_order, cat_id ,goods_number, integral, " .
 ...
 */
function check_skip_link() {
	$config_file = $GLOBALS['config_file'];
	$domain = array(
		'new3g.chinaskin.cn',//3G站
		'www.sell3.com',//Q站
		'www.chinaskin.cn',//官网
	);
	return in_array($config_file, $domain);
}
 ...
 *
 */
function reformat_image_name($type, $goods_id, $source_img, $position='', $target_sub_dir='')
{
...
function get_cat_goods($cat_id){
    $sql = "SELECT * FROM  " . $GLOBALS['ecs']->table('goods_cat') . " WHERE cat_id = '$cat_id'";
    $rows = $GLOBALS['db']->getAll($sql);
    foreach ($rows as $val){
        $goods[] = $val['goods_id'];
    }
    
    return empty($goods) ? 0 : implode(',',$goods);
}

function copy_info($goods_id,$copy_id,$config_file){
    $db = $GLOBALS['db'];
    $ecs = $GLOBALS['ecs'];
    //检查通用信息  查找goods表 , 获取 cat_id ,keywords , goods_type , virtual_sale , market_price
    $sql = "SELECT cat_id ,keywords , goods_type , virtual_sale , market_price ,brand_id FROM " . $ecs->table('goods') . " WHERE goods_id = '$copy_id'";
    $goods = $db->getRow($sql);
    if (!empty($goods)){
        $db->autoExecute($ecs->table('goods'), $goods, 'UPDATE',"goods_id = ".$goods_id);       //修改goods表

        //==扩展分类==    --- 扩展分类, 产品分类 ,皮肤分类 , 功效分类 等
        $sql = "SELECT  cat_id  FROM " . $ecs->table('goods_cat') . " WHERE goods_id = '$copy_id'";
        $res  = $db->getAll($sql);
        if (is_array($res) ){
            ...
            foreach($res as &$v){
                $v['goods_id'] = $goods_id;
//               $db->autoExecute($ecs->table('goods_cat'), $v, 'INSERT');       //修改goods_cat表(商品扩展分类)
            }
            $db->batchInsert($ecs->table('goods_cat'),$res);                      //批量添加
        }

        //==商品属性==   -- 功效 ,适用肤质等
        $sql = "SELECT  attr_id , attr_value , attr_price  FROM " . $ecs->table('goods_attr') . " WHERE goods_id = '$copy_id'";
        $res  = $db->get_addslashes_all($sql);
        if (is_array($res)){
            $sql = "DELETE  FROM " . $ecs->table('goods_attr') . " WHERE goods_id = '$goods_id'";
            $db->query($sql);
            foreach($res as &$v){
                $v['goods_id'] = $goods_id;
//                $db->autoExecute($ecs->table('goods_attr'), $v, 'INSERT');       //修改goods_attr表(商品属性)
            }
            $db->batchInsert($ecs->table('goods_attr'),$res);                      //批量添加
        }

        //==商品评论==  -PC、Q站&3G的商品评论    comments表  ecs_comments_gallery表->comments_id
        $is_exists_comments_gallery = $db->getRow("show tables like '%comments_gallery'")    ;        //评论图片表  Q和3g有 pc没有这个表
        if ($config_file =='www.chinaskin.cn' || $config_file == 'www.chinaskin.net'){               //评论表 q,3g 和 pc 的不一样
            $sql = "SELECT  *  FROM " . $ecs->table('comment') . " WHERE id_value = '$copy_id'";
            $res  = $db->get_addslashes_all($sql);
            if(false !== $res){
                $sql = "DELETE  FROM " . $ecs->table('comment') . " WHERE id_value = '$goods_id'";             //删除评论
                $db->query($sql);
                foreach($res as &$v){
                    unset($v['comment_id']);                                //去除自增主键
                    $v['id_value'] = $goods_id;
//                    $db->autoExecute($ecs->table('comment'), $v, 'INSERT');       //修改comments表(商品评论)
                }
                $db->batchInsert($ecs->table('comment'),$res);                      //批量添加
            }
        }
        else {                                                                                                              // 3G|Q
            $sql = "SELECT  *  FROM " . $ecs->table('comments') . " WHERE id_value = '$copy_id'";
            $res  = $db->get_addslashes_all($sql);
            if(false !== $res){                                                             //有数据
                //=== 删除原数据===
                $sql = "SELECT  id  FROM " . $ecs->table('comments') . " WHERE id_value = '$goods_id'";
                $delete_list  = $db->getCol($sql);                                          //获取主键id用于删除评论对应图片
                $sql = "DELETE  FROM " . $ecs->table('comments') . " WHERE id_value = '$goods_id'";             //删除评论
                $db->query($sql);
                if($delete_list){
                    $sql = "DELETE  FROM " . $ecs->table('comments_gallery') . " WHERE comment_id " . db_create_in($delete_list);         //删除商品评论图片表
                    $db->query($sql);
                }
                //=== 添加新数据====
                foreach($res as $v){
                    if($is_exists_comments_gallery){                                                                //有商品评论图片表
                        $sql = "SELECT  *  FROM " . $ecs->table('comments_gallery') . " WHERE comment_id = '$v[id]'";
                        $comments_gallerys = $db->get_addslashes_all($sql);
                        unset($v['id']);                                            //去除自增主键
                        $v['id_value'] = $goods_id;
                        if($db->autoExecute($ecs->table('comments'), $v, 'INSERT')){       //修改comments表(商品评论)
                            $comments_id = $db->insert_id();
                            if(is_array($comments_gallerys)){
                                foreach($comments_gallerys as $comments_gallery){                       //修改comments_gallery表(商品评论图片)
                                    unset($comments_gallery['id']);
                                    $comments_gallery['comment_id'] = $comments_id;
                                    $db->autoExecute($ecs->table('comments_gallery'), $comments_gallery, 'INSERT');
                                }
                            }
                        }
                    }
