...
class UserManageModel extends CpanelUserCenterModel
{
    ...
    public function filter(&$params){
        ...
        if(!empty($params['keyword'])){
            ...
            switch ($params['type']) {
                ...
                case 6:
					...
					break;
				case 7:
					$where['user.custom_id'] = $keyword;
					break;
                default:
                    $where['_string'] = "(user.user_id='$keyword' or user.email like '%$keyword%' or user.mobile='" . PhxCrypt::phxEncrypt($keyword) . "' or user.user_num='$keyword' or user.user_name like '%$keyword%') or user.custom_id='$keyword'";
            }
        }
        //上次登陆时间
        ...
        $fields = 'user.user_id,user.auto_reg_time,user.email,user.mobile,user.user_num,user.user_name,user.custom_id,user.custom_no,user.state,user.reg_time,user.last_time,account.rank,info.source';
        $this->alias('user')
