...
class RotateController extends CpanelController {
    ...
    public function __construct() {
        ...
        $data_power = getDataPower(login('user_id'), 'WechatAccount');  //获取数据权限
        $this->account_id = I('request.account_id','','trim');
        if(empty($this->account_id)){  //没有传account_id，获取当前默认
            if(!empty($data_power)){
                $this->account_id = D('WechatAccount')->where((!empty($data_power) ? "id IN(".implode(',',$data_power).")" : ''))->getField('id');
            }else{
                $this->account_id = D('WechatAccount')->where((!empty($data_power) ? "id IN(".implode(',',$data_power).")" : ''))->getField('id');
            }
            if($this->account_id){
                ...
                $this->account_id = 0;  //不允许所有公众号
            }
        }
        if(!empty($data_power) && !in_array($this->account_id, $data_power)){
            $this->error('没权限操作此公众号');
        }
    }

    public function _before_index(){
        $data_power = getDataPower(login('user_id'), 'WechatAccount');  //获取数据权限
        $account_list = D('WechatAccount')->field('id,text')->where((!empty($data_power) ? "id IN(".implode(',',$data_power).")" : ''))->select();
    ...
    public function _after_before($params){
        ...
        if(isset($params['account_id'])){
            $act_data['account_id'] = intval($params['account_id']);
        }
//        echo '<pre>';
//        print_r($params);exit;
        if(!empty($params['id'])){
