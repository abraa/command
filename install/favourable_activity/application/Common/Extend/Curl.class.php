...
class Curl
{
     ...
     * 模拟GET的方法
     * @param $url
     * @param string $cacertUrl 证书路径
     * @return mixed
     */
    static function get($url, $cacertUrl = '')
    {
        $ch = curl_init($url);
     ...
     *
     */
    static function post($url, $data, $dataType = '')
    {
        $ch = curl_init($url);
        curl_setopt($ch, CURLOPT_HTTPHEADER, self::$headers);
        curl_setopt($ch, CURLOPT_HEADER, 0);
        ...
        curl_setopt($ch, CURLOPT_TIMEOUT, self::$timeOut);
        if (self::$proxy) curl_setopt($ch, CURLOPT_PROXY, self::$proxy);
        curl_setopt($ch, CURLOPT_POSTFIELDS, (strtolower($dataType) == 'json' ? json_encode($data) : (is_array($data) ? http_build_query($data) : $data)));
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
     ...
     * @return bool|mixed
     */
    static public function postJson($json, $url) {
        $ch = curl_init($url);
        curl_setopt($ch, CURLOPT_HTTPHEADER, self::$headers);
        curl_setopt($ch, CURLOPT_HEADER, 0);
        curl_setopt($ch, CURLOPT_USERAGENT, self::$user_agent);
