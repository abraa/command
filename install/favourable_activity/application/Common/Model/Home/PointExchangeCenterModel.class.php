 ...
 * Date: 2018-01-22 16:03
 * ====================================
 * File: PointExchangeCenterModel.class.php
 * ====================================
 */
...
class PointExchangeCenterModel extends UserCenterModel{
    ...
    public function getPage($user_id = 0, $page = 1, $pageSize = 0) {
		...
		$result = D('Common/Home/IntegralCenter')->getPointsLeft($user_id);  //查询用户可用积分, 会计算被冻结的积分在内
	...
	private function _checkPackage($goods_id = 0) {
		...
		$GoodsModel = D('Common/Home/GoodsCenter');
	...
	private function _getGoodsAttr($goods_id) {
		$GoodsAttrModel = D('Common/Home/GoodsAttrCenter');
	...
	private function _getAttrInfo() {
		$AttributeModel = D('Common/Home/AttributeCenter');
	...
	private function _getPackageInfo($package_id) {
		$PackageGoodsModel = D('Common/Home/PackageGoodsCenter');
	...
	private function _getGoodsImg($goods_id) {
		$rows = D('Common/Home/GoodsGalleryCenter')->field('img_url,img_desc')->where("goods_id=".$goods_id)->select();
        return $rows;
    }
