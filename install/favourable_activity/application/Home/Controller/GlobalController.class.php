...
class GlobalController extends InitController
{
    ...
    public function cartGoodsNum() {
        //检查并且删除不可卖的商品
        D('Common/Home/Cart')->checkCartGoods();
        //检查购物车活动商品是否可买
        $cartLogic = new \Common\Logic\Cart();
        $cartLogic->checkCartActivity();
        $count = D('Common/Home/Cart')->getCartCount();
        $this->success(array('cartGoodsNum' => $count));
    }
    ...
    public function getAdvisoryInfo()
    {
        ...
        $favourableActivity = new FavourableActivityLogic(array("1"));                // --add by abraa
        if($favourableActivity->checkFavourableActivityStarted()){
            $data['activity'] = true;
        }else{
            $data['activity'] = false;
        }
