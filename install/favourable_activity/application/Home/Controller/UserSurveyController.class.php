...
class UserSurveyController extends InitController
{
    ...
    public function survey(){
        ...
        $channel    = !empty($params_data[1]) ? $params_data[1] : '0305 ';; //评价渠道:01定单评价,0201呼叫中心呼出,0202呼叫中心呼入,0301微信,0302个人QQ,0303企点QQ,0304营销QQ,0305公众号
        ...
        $custom_no  = !empty($params_data[6]) ? $params_data[6] : ''; //客户编号
		$remarks    = !empty($params_data[7]) ? $params_data[7] : ''; //工单相关单号
...
//            F('survey_log'.date('Y-m-d'), var_export(array($this->openId=>array($evalDime,$name,$channel,$evalPerson), 'time'=>date('Y-m-d H:i:s')), TRUE)."\n".F('survey_log'.date('Y-m-d')), $path=DATA_PATH);
		...
		foreach($comments as $key=>$val){
			if(empty($val['content']) && !empty($val['scope'])){
				...
				switch($evalDime){
					case '0201'://售前
                        $content = !empty($remarks) ? '工单评价' : '售前服务';
						break;
					case '0202'://售中
						$content = '下单便利性';
						break;
					case '0203'://售后
						$content = !empty($remarks) ? '工单评价' : '售后服务';
						break;
					case '0103'://价格
						$content = '价格服务';
						break;
        ...
        //TODO:积分操作，根据后台设置，评价完后得到相对应积分增加
        $this->addIntegral($evalDime, $custom_no);
    ...
    /**
     * 评价积分操作，根据后台设置，评价完后得到相对应积分增加
     * @param $evalDime 评价维度
     * @param $custom_no 客户编号
     */
    protected function addIntegral($evalDime, $custom_no=''){
//        $custom_no = '1600000001';
        $integral = new Integral();
        $extra = array();
        $extra['type'] = 2;
        if(in_array($evalDime, array('0101'))){

            //满意度问卷调查
            $remark = '品牌调查获赠积分';
            $extra['extend_type'] = 4;
//            $extra['user_id'] = 6653;
            $integral->newVariety(C('SITE_ID'), 0, $remark, 0, false, $extra);

        }else if(in_array($evalDime, array('0201', '0203'))){

            //TODO:只有在有客户编号的情况下才进行加积分操作
            if(!empty($custom_no)){
                // 0201-售前，0203-售后
                switch($evalDime){
                    case '0201': //0201售前
                ...
                $remark = '服务评价获赠积分';
                $userExtend = new User();
                $user_id = $userExtend->getUserIdByCustomNo($custom_no);
                $extra['user_id'] = $user_id;
                $res = $integral->newVariety(C('SITE_ID'), 0, $remark, 0, false, $extra);
            }
