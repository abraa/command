...
class RotateController extends InitController {
    ...
    public function rotating(){
        ...
        if(!$is_winner){
			...
			foreach($new_prizes as $n_k=>$val){
                if(!empty($val['id']) && $val['grade']<=0){
                    unset($new_prizes[$n_k]);
                    $chance_int = $val['chance_int'];
                    continue;
                }
				if($val['grade'] === 0){
    ...
    public function goodsInfo(){
        ...
        if($rotate_id && $prize_id){
            ...
            if($rotate_info['shipping_free']){
                $logicUserAddress = new \Common\Logic\UserAddress();  //用户收货地址业务逻辑层
                $PaymentObject = new \Common\Logic\Payment();
                //检查收货地址是否有设置
                $consignee = $logicUserAddress->getUserAddress($address_id);
                //计算邮费
                $province = isset($consignee['province']) ? $consignee['province'] : 0;
                $rotate_info['shipping_free'] = $PaymentObject->getShippingFee(0, $province, $rotate_info['postage'], false);
            }else{
...
//            $goods_info['shop_price'] = sprintf("%d",$goods_info['shop_price']);
	...
	public function buildOrder(){
		...
		$count = D('Common/Home/OrderInfo')->where(array('add_time'=>array('gt',$limit_time), 'ip_address'=>$real_ip, 'postscript'=>array('like','%中奖订单%')))->count();
        ...
        $PaymentObject = new \Common\Logic\Payment();
        $PaymentObject->setData('address_id', $address_id);
        $PaymentObject->setData('payment_id', $payment_id);
        $PaymentObject->setData('remark', $remark);
        $PaymentObject->setData('source_url', I('request.source_url','','trim'));
        $logicUserAddress = new \Common\Logic\UserAddress();  //用户收货地址业务逻辑层
        ...
        if($rotate_info['shipping_free']){
            $province = isset($consignee['province']) ? $consignee['province'] : 0;
            $data['shipping_fee'] = $PaymentObject->getShippingFee(0, $province, $rotate_info['postage'], false);
        }else{
        ...
        $order = array(
            ...
            'site_id'         => $site_id,
            'extension_code'  => CAT_LOTTERY_BUY,
            'extension_id'    => $rotate_id,
        );

        //获取支付信息
        $paymentData = $PaymentObject->getPaymentData();
        $order = array_merge($order, $paymentData);

        //获取订单的来源地址
        $order = $PaymentObject->getOrderSource($order);
        ...
        if($order['order_id']){
            ...
            $orderGoodsModel->addAll($package_goods_data);//插入商品数据到当前站点数据库
            $orderGoodsCenterModel->addAll($package_goods_data);  //插入商品数据到ucenter数据库
        }else{
            $this->error('提交订单失败');
        }
        ...
        if ($order['order_amount'] > 0){
            ...
            D('Common/Home/PayInfo')->add($pay_info_data);
        }
        $data = array(
            'order_sn'=>$order['order_sn'],
            ...
            'content'=>isset($order['content']) ? $order['content'] : '',
        );
