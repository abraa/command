...
class  RightsController extends InitController
{
    ...
    public function getList()
    {
        ...
        $data = $rightsModel
            ->page($page, $pageSize)
            ->where($where)
            ->order('rid DESC')->field('rid,complaint_no,follow_kefu,create_time,status,order_sn,mobile,remark')
            ->select();
        if (empty($data)) {
            $this->error('暂无维权数据！');
        }
    ...
    public function confirm(){
        ...
        $rightsModel = D('Rights');
        $data = $rightsModel
            ->field('rid,complaint_no,follow_kefu,create_time,status,order_sn,mobile,remark')
            ->where(array('rid' => $rid))
            ->find();
    ...
    private function setPoint(){
        ...
        $integral->newVariety($site_id,0,'维权评价积分',0,false,$extra);
    }
