...
class UserController extends InitController {
            ...
            if($ret){
                ...
                if($user_id > 0){
                    ...
                    if($points > 0 && $is_merge_integral <= 0){
                        ...
                        $result = $IntegralObject->newVariety(C('SITE_ID'), $points, '签到积分到账', 0, false, array('user_id'=>$user_id,'type' => 1));
                        if($result){
    ...
    */
	public function resetPassword(){
        ...
        //重置自动注册的用户状态为正常状态
        $this->resetState($user_id);

		$this->success();
	}
    //重置自动注册的用户状态为正常状态
    public function resetState($user_id){
        $userInfo = session('userInfo');
        if(!empty($userInfo[$user_id]) && $userInfo[$user_id]['state'] == 1){
            $this->_muser->where(array('user_id'=>$user_id))->setField('state', 0);
            $userInfo[$user_id]['state'] = 0;
