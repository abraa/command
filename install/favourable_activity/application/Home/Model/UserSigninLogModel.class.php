...
class UserSigninLogModel extends CommonModel
{
    ...
    public function getSigninPoints($bind_id)
    {
        ...
        if ($result && !empty($user_id)) {
            ...
            $integra->newVariety(C('SITE_id'), $add_points, '签到积分', 0, false, array('user_id' => $user_id, 'type' => 1));
        }

        $total_point = $this->totalPoint($bind_id);
        return array('days' => $days, 'add_points' => $add_points,'total_point'=>$total_point);
    }
     ...
     * @return bool|mixed
     */
    private function totalPoint($bind_id)
    {
        $totalPoint = D('BindUser')->where(array('bind_id' => $bind_id))->getField('points_left');
        if (empty($totalPoint)) {
            return false;
        }
