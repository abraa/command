	...
	*/
    private function getStatus($order){
		...
		if($order['shipping_status'] != SS_UNSHIPPED){
			...
			if ($order['shipping_status'] == SS_RECEIVED && ($order['add_time'] > Time::localStrtotime('-3 Month'))) {  //已签收
				...
				if(!$is_comment){
					...
					//表示已评价
					$ret['handle'] = 3;
                    //判断已评价的商品是否可追评，只要订单的商品中有一条商品没追评，就显示可追评状态
                    $r = $this->enCommentAdd($order['order_sn']);
                    if($r){
                        //可追评
                        $ret['handle'] = 4;
                    }
     ...
     * @return bool
     */
    public function enCommentAdd($order_sn){
        $where['order_sn'] = $order_sn;
        $where['site_id'] = C('SITE_ID');
        $where['parent_id'] = 0;
        $order_goods = D('OrderGoodsCenter')->where($where)
            ->field('goods_id,extension_code')
            ->select();
        if($order_goods){
            ...
            foreach($order_goods as $val){
                if($val['extension_code'] == 'package_buy'){
                    //套装,查找goods_id
                    $goods_id = $goodsActivityModel->getPackageGoodsId($val['goods_id']);
                }else{
                    //单品
                ...
                if($goods_id){
                    ...
                    $map['is_client'] = 1;
                    $z_date = $commentsModel->where($map)->getField('z_date');
                    if(empty($z_date)){
                        return true;
                    }
                }
