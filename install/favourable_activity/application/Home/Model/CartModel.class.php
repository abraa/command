...
class CartModel extends CommonModel
{
    ...
    */
     ...
     * @return array
     */
    function changeDataToCart($add_goods_number, $goods_info = array(), $package_info = array(), $user_id = 0)
     ...
     * @param $goods_number
     */
    public function updateToCart($rec_id, $goods_number, $user_id = NULL)
    {
        $sql_data = 'goods_number = goods_number+' . $goods_number . ',session_id="' . session_id() . '"';
        if(!is_null($user_id)){
            $sql_data .= ',user_id = ' . $user_id;
        }
        return $this->execute('update __TABLE__ set ' . $sql_data . '  where rec_id = ' . $rec_id);
    }
    ...
    public function getPackageInfo($goods_id, $time = 0, $act_id = 0)
    {
        ...
        if (empty($goods_id) && $act_id > 0) {
            ...
            $package_info = M('GoodsActivity')
                ->where(array("goods_id" => $goods_id,'start_time' => array('egt',$time),'end_time' => array('lt',$time)))
                ->find();
        }
    ...
    public function getGoodsData($goods_id)
    {
        ...
        return D('goods')->field('goods_id,cat_id,goods_name,goods_number,goods_sn,is_on_sale,market_price,shop_price')->where('goods_id = ' . $goods_id)->find();

    }

    public function goodsExtCat($goods_id)
    {
        $cats = array();
        $ext_cat = D('goods')->field('cat_id')->where(array('goods_id' => $goods_id))->select();
        if (!empty($ext_cat)) {
            foreach ($ext_cat as $v) {
                $cats[] = $v['cat_id'];
            }
        }
        $goods_cat = M('goods_cat')->where(array('goods_id' => $goods_id))->select();
        if(!empty($goods_cat)){
