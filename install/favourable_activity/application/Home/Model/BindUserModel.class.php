...
class BindUserModel extends CommonModel {
    ...
    public function SignIn($data){
        ...
        //未绑定手机号的先绑定手机号
        if(strtolower(CONTROLLER_NAME) == 'wechatdrainage' && empty($row['mobile'])){
            return "您好，请先绑定您的手机号码\n<a href='http://q.chinaskin.cn/#/check-code?openid={$openid}'>（手机认证）</a>";
        }

        $userSigninLogModel = D('UserSigninLog');
        $signInResult = $userSigninLogModel->getSigninPoints($row['bind_id']);
        $domain = 'http://q.chinaskin.cn';

        if(CONTROLLER_NAME == 'WeChatDrainage'){
            $url = "<a href='".$domain."/#/checkin/".$row['bind_id']."?openid={$openid}'>点此查看详情</a>";
        }else{
        ...
        if($signInResult){
           if(CONTROLLER_NAME == 'WeChatDrainage'){
               if(empty($signInResult['total_point'])){
                   $signInResult['total_point'] = 0;
               }
               $signInResult['total_point'] += $signInResult['add_points'];
               $result  = $row['nickname']."您好，签到成功！\n";
               $result .= "您已连续签到: ".$signInResult['days']." 次\n";
               $result .= "本次签到获得: ".$signInResult['add_points']."积分\n";
               $result .= "目前累计积分：".$signInResult['total_point']."积分\n";
               $result .= "每天坚持签到，获取更多积分，明天等你来哦！\n>> ";
               $result .= $url;
           }else{
               $result = $row['nickname']."您好，签到成功！\n";
               $result .= "您已连续签到 ".$signInResult['days']." 次。\n";
