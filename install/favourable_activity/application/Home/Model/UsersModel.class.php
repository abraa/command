...
class UsersModel extends UserCenterModel {
	...
	*/
	public function getUserLoginInfo($user_id = 0){
		...
		if(!empty($row)){
			//获取当前总积分和等级
            $rank = $this->getUserRank($row['custom_id']);
            $row = array_merge($row,$rank);

			$result = D('IntegralCenter')->getPointsLeft(0, $row['custom_id']);  //查询用户可用积分, 会计算被冻结的积分在内
     ...
     * 获取客户的积分详情
     */
    private function getUserRank($custom_id = 0){
        $row = array();
        //获取当前总积分和等级
        $Integral = D('IntegralCenter')->field('total_points,rank')->where("customer_id = '$custom_id' AND is_invalid = 0")->find();
        $rank_id = isset($Integral['rank']) ? $Integral['rank'] : 0;
        $row['total_points'] = isset($Integral['total_points']) ? $Integral['total_points'] : 0;
        if($rank_id > 0){
            $row['rank_name'] = D('UserRank')->where(array('rank_id'=>$rank_id))->getField('rank_name');
            if(is_null($row['rank_name'])){
                $UserRankInfo = D('UserRank')->field('rank_id,rank_name')->order('rank_name asc,rank_id asc')->find();
