...
class GoodsModel extends CommonModel{
	...
	public function getList($goods_id = NULL, $price = NULL, $keyword = NULL, $cate_id = 0, $is_package = 0, $page = 1, $pageSize = 8){
		...
		$goods = $this
			->alias('g')
			->field('g.goods_id,g.goods_sn,g.skip_link,g.cat_id,g.is_real,g.goods_name,g.is_package,g.click_count,g.market_price,g.shop_price,g.shop_price as goods_price,g.promote_price,g.goods_thumb,g.goods_img,g.original_img,g.goods_guanggao,g.virtual_sale,ga.attr_value')
	...
	public function getSpecialList($goods_id = NULL){
        ...
        */
		...
		$this->field("g.goods_id, g.goods_name, g.market_price, g.is_package, IFNULL(mp.user_price, g.shop_price * '$member_discount') AS shop_price, g.goods_thumb, g.goods_img,g.original_img,g.skip_link");
		...
		$this->where("g.is_on_sale = 1  AND g.is_delete= 0 AND g.is_series = 1 AND g.goods_id IN(" . $goods_id . ")");  // AND g.is_best = 1
		$list = $this->select();
		...
		if(!empty($list)){
			...
			foreach($list as $key=>$row){
				...
				$row["goods_img"] = !empty($row["goods_img"]) ? C('domain_source.img_domain').$row["goods_img"] : '';
				$row["original_img"] = !empty($row["original_img"]) ? C('domain_source.img_domain').$row["original_img"] : '';
				$row["goods_thumb"] = !empty($row["goods_thumb"]) ? C('domain_source.img_domain').$row["goods_thumb"] : '';
	...
	public function getGoodInfo($goods_id){
		...
		$field .= 'g.is_promotion, g.is_package, g.goods_guanggao, g.virtual_sale, g.is_on_sale, g.skip_link, ';
     ...
     * @return array
     */
    public function getGoodsCatGoodsId($cat_id = ''){
        $where = array();
        if(strstr($cat_id, ',') === false){
            $where['cat_id'] = array('IN',$cat_id);
        }else{
            $where['cat_id'] = $cat_id;
        }
        $rows = M('GoodsCat')->field('goods_id')->where($where)->select();
        $goods_id = array();
        foreach ($rows as $val){
    ...
    /**
     * 获取所有子分类ID
     * @param string $parent_id 上级ID，多个之间逗号隔开
     * @return string
     */
    public function getCatChildrenId($parent_id = ''){
        $cat_id = array($parent_id);
        $where = array();
        if(strstr($parent_id,',')){
        ...
        $cates = M('Category')->where($where)->field('cat_id')->order("cat_id ASC")->select();
        if(!empty($cates)){
            foreach($cates as $value){
                $cat_id[] = $this->getCatChildrenId($value['cat_id']);
            }
        }
    ...
    /**
     * 获取某个分类下的goods_id，包括子分类
     * @param int $cat_id
     * @return array
     */
    public function getCatGoodsId($cat_id = 0){
        if($cat_id <= 0){
            return array();
        }
        $cat_id = $this->getCatChildrenId($cat_id);  //获取所有子分类ID
        $goods = $this->field('goods_id')->where("cat_id IN(".$cat_id.")")->select();
        $goods_id = array();
        if(!empty($goods)){
